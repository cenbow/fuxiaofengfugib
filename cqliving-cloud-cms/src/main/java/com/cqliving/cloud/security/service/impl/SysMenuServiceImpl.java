...
import com.cqliving.cloud.online.security.dao.SysMenuDao;
...
public class SysMenuServiceImpl extends EntityServiceImpl<SysMenu, SysMenuDao> implements SysMenuService {
    ...
    @Override
    public SysMenu save(SysMenu o) throws BusinessException {
        super.save(o);
     ...
     * @return
     */
   /*private List<MenuView> queryAuthorisedMenus(List<MenuView> list){
        ...
        Iterator<MenuView> ie = list.iterator();
        //组装二级菜单的展示对象
        while (ie.hasNext()) {
            //Object[] obj = (Object[]) ie.next();
        	MenuView menuViewDto = ie.next();
           // SysResource resource = (SysResource) obj[0];
           // SysMenu menu = (SysMenu) obj[1];
        	SysMenuDto menu = menuViewDto.getSysMenuDto();
        	SysResourceDto resource = menuViewDto.getSysResourceDto();
            if(menuViewMap.containsKey(menu.getParentId())==false) {
                menuViewMap.put(menu.getParentId(), new LinkedList<MenuView>());
            }
            MenuView subMenuView = new MenuView(menu.getId(), menu.getTitle(),menu.getIcon());
            subMenuView.getAttributes().put("url",resource.getResString());
    ...
    public List<MenuView> queryAuthorisedMenus(String userName) {
        return super.getEntityDao().queryAuthorisedMenusByUserName(userName);
    }

    @Override
    public List<MenuView> queryAuthorisedMenus(Long roleId) {
        List<MenuView> list = securityCache.getMenu(roleId);
        if(list == null && (list = super.getEntityDao().queryAuthorisedMenusByRole(roleId))!=null){
	...
	public List<SysMenuDto> findAllSysMenu() {
		...
		return this.getEntityDao().findAllSysMenu();
	}
