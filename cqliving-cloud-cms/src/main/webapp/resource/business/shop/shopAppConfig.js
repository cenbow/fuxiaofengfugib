define(["cqliving_dialog", "cqliving_ajax", "chosen"], function(cqDialog, cq_ajax) {
	return {
		init: function() {
			...
			$("#appId").change(function() {
				var appId = $(this).val();
				location.href = "/module/shop/shop_app_config.html?appId=" + appId;
			});
		}
	};
	...
	function configSave() {
		cqDialog.confirm("操作确认", "确定保存设置吗？", function() {
			cq_ajax.ajaxOperate("/module/shop/shop_app_config_save.html", null, $("#myForm").serialize(), function(data, status) {
				if (data.code >= 0) {
