define(["cloud.table.curd", "cloud.time.input", "cqliving_dialog", "cqliving_ajax"], function(tableCurd, timeInput, cqDialog, cq_ajax) {
	return {
		init: function() {
			...
			//批量发布
			$("#publishBatchButton").click(publishBatch);
//			//上线
...
//			$("#table_content_page").on("click", ".offline_btn", offline);
	...
	function offlineBatch() {
		...
		cqDialog.confirm("操作确认", "确定要下线吗？", function() {
			cq_ajax.ajaxOperate("/module/joke/joke_info_offline_batch.html", null, {"ids": ids}, function(data, status) {
				if (data.code >= 0) {
	...
	function publishBatch() {
		...
		cqDialog.confirm("操作确认", "确定要发布吗？", function() {
			cq_ajax.ajaxOperate("/module/joke/joke_info_publish_batch.html", null, {"ids": ids}, function(data, status) {
				if (data.code >= 0) {
					cqDialog.success(data.message ? data.message : "操作成功");
					$("#searchButton").trigger("click");
				} else {
