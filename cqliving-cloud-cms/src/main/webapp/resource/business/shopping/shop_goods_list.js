define(['validator.bootstrap','cloud.table.curd','cqliving_ajax','cqliving_dialog','chosen'], function($,tableCurd,cqliving_ajax,cqliving_dialog){
	...
	function loadInit(){
		...
		//修改分类，刷新列表
		$("body").on("change","#search_EQ_shoppingCategoryId",function(){
			$("#searchButton").click();
		});
	 ...
	 */
	function recommemd(){
		$('body').on('click','.recommemd',function(){
			var jThis = $(this);
			var id = jThis.attr('goodsId');
			var url = jThis.attr("url");
			var tip = jThis.attr('tip');
			cqliving_dialog.confirm('操作确认',tip,function(){
				cqliving_ajax.ajaxOperate(url,jThis,{"id":id},function(data,status){
					if(data.code >= 0){
						cqliving_dialog.success(data.message?data.message:"操作成功",function(){
							$("#searchButton").trigger("click");
						});
	 ...
	 * 商品分类下拉选项加载
	 */
	function loadShoppingCategory(){
		var appId = $("#search_EQ_appId").val();
		var url='/module/shopping_category/common/getLevelOne.html';
		$('#shopping-category-div').show();
		var data = {};
		data['appId'] = appId;
		cqliving_ajax.load($('#shopping-category-div'),url,data,function(){});
	}
