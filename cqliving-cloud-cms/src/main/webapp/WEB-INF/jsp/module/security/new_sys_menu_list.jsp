...
<!-- //////////////////////////////////////////////////////////////////////////// -->
    <script type="text/javascript">
        ...
        require(['angular', 'angular.ui.tree', 'jquery', 'underscore', 'util'], function(angular, tree, $, _, util){
            angular.module('app', ['ui.tree'], function($httpProvider) {
                ...
                $scope.sysResTypes = ${sysResTypesJson};
                
                $scope.getResc = function(resType){
                	return $scope.rescs[resType.id];
                }
                
                $scope.switchHasSub = function(rescs){
                	
                	var h = 1;
                	if(rescs && rescs.length>=1)return h=1;
                	return h=2;
                	
                }
                
                $scope.isNotEmpty = function(rescs){
                	
                	if(rescs && rescs.length>=1)return true;
                ...
                $scope.iteratesResc = function(rescs, callback, parent, level){
                    ...
                    for(var i in rescs){
                    	...
                    	for(var j=0;j<res.length; j++){
                            ...
                            if(val.subResource && val.subResource.length>0){
                ...
                $scope.showIconDialog = function(elm) {
                    ...
                    util.iconBrowser(function(ico){
                    	...
                    	ico = ico.replace("fa","").replace("fa","icon");
                ...
                $scope.showDetailDialog = function(menu) {
                    ...
                    if(menu.resourceId!=""){
                        $scope.iteratesResc($scope.activeRescs, function (val) {
                            ...
                            if(menu.resourceId==val.id){
                                val.check = true;
                                $(".tab-content :input[value="+val.id+"]").prop("checked",true);
                            }
                        });
                    }
                ...
                $scope.deleteMenu = function(menu) {
                    if(!confirm('将删除菜单，'+(menu.subSysMenu && menu.subSysMenu.length > 0 ? '同时删除所有子菜单,' : '') + '是否继续? ')){
                        return;
                    }
                            	...
                            	if(dat.code>=0){
                            		...
                            		if(menu.parentId==0){
                                        ...
                                        angular.forEach($scope.menus, function(val){
                                            angular.forEach(val.subSysMenu, function(sub){
                                                if(sub.id==menu.id){
                                                    val.subSysMenu = _.without(val.subSysMenu, menu);
                                                    return;
                                                }
                                            });
                                        });
                        ...
                        if($scope.activeMenu.parentId==0){
                            ...
                            angular.forEach($scope.menus, function(val){
                                if(val.id==$scope.activeMenu.parentId){
                                    val.subSysMenu = val.subSysMenu ? val.subSysMenu : [];
                                    menuList=val.subSysMenu;
                                    return;
                                }
                            });
                        }
                ...
                $scope.treeOptions = {
                    dragStop: function(event) {
                        ...
                        angular.forEach($scope.menus, function(menu, index){
                            ...
                            if(menu.subSysMenu){
                                angular.forEach(menu.subSysMenu, function(sub, index){
                                    sub.sortNum=index+1;
  ...
  <div class="main-content">
	<jsp:include page="${pageContext.request.contextPath}/WEB-INF/jsp/common_ace/breadcrumbs.jsp">
	   <jsp:param value="菜单管理" name="_breadcrumbs_1"/>
	</jsp:include>
        ...
        <div class="ng-scope ng-cloak" ng-controller="menuDesigner">
            <div class="panel panel-default">
                <div class="panel-heading">
                    ...
                    <cqliving-security2:hasPermission name="/module/security/sys_menu_add.html">
                           <button type="submit" class="btn btn-sm btn-primary" ng-click="addMenu();">增加菜单</button>
                    ...
                    <ol ui-tree-nodes ng-model="menus" data-type="group">
                        <li ng-repeat="menu in menus" ui-tree-node>
                            <div class="group-title angular-ui-tree-handle">
                                <cqliving-security2:hasPermission name="/module/security/sys_menu_del.html">
                                <a href="" class="btn btn-danger btn-xs pull-right" data-nodrag ng-click="deleteMenu(menu)"><i class="icon-trash bigger-130"></i></a>
                                </cqliving-security2:hasPermission>
                                <cqliving-security2:hasPermission name="/module/security/sys_menu_update.html">
                                <a href="" class="btn btn-primary btn-xs pull-right" data-nodrag ng-click="showDetailDialog(menu);"><i class="icon-pencil bigger-130"></i></a>
                                </cqliving-security2:hasPermission>
                                <cqliving-security2:hasPermission name="/module/security/sys_menu_view.html">
                                <a href="" class="btn btn-primary btn-xs pull-right" data-nodrag ng-click="showViewDialog(menu);"><i class="icon-zoom-in bigger-130"></i></a>
                                </cqliving-security2:hasPermission>
                                <div>
                            ...
                            <ol ui-tree-nodes ng-model="menu.subSysMenu" data-type="category" ng-class="{hidden: collapsed}">
                                <li ng-repeat="sub in menu.subSysMenu" ui-tree-node>
                                    <div class="category-title angular-ui-tree-handle">
                                        <cqliving-security2:hasPermission name="/module/security/sys_menu_del.html">
                                        <a href="" class="btn btn-danger btn-xs pull-right" data-nodrag ng-click="deleteMenu(sub)"><i class="icon-trash bigger-130"></i></a>
                                        </cqliving-security2:hasPermission>
                                        <cqliving-security2:hasPermission name="/module/security/sys_menu_update.html">
                                        <a href="" class="btn btn-primary btn-xs pull-right" data-nodrag ng-click="showDetailDialog(sub);"><i class="icon-pencil bigger-130"></i></a>
                                        </cqliving-security2:hasPermission>
                                        <cqliving-security2:hasPermission name="/module/security/sys_menu_view.html">
                                        <a href="" class="btn btn-primary btn-xs pull-right" data-nodrag ng-click="showViewDialog(sub);"><i class="icon-zoom-in bigger-130"></i></a>
                                        </cqliving-security2:hasPermission>
                                        <div>
                                ...
                                <li class="angular-ui-tree-node">
                                    <div class="category-title angular-ui-tree-handle">
                                        <form class="form-inline" role="form">
                                            <button type="submit" class="btn btn-sm btn-primary" ng-click="addMenu(menu);">增加子菜单</button>
                                        </form>
                                    </div>
                                </li>
                            </ol>
                    ...
                    <div class="modal-content">
                        <form class="form-horizontal form" method="post" id="form1" name="form1" ng-submit="saveMenu(form1.$valid)"  novalidate>
                            ...
                            <div class="modal-body">
                                    ...
                                    <div class="col-sm-3">
                                        <div class="input-group">
                                            <div class="input-group" style="width: 300px;">
                                                ...
                                                <span class="input-group-addon" style="display:{{activeMenu.icon ? '' : 'none'}};"><i class=" fa {{activeMenu.icon}} "></i></span>
                                                <span class="input-group-btn">
                                                    <button class="btn btn-sm btn-primary" type="button" ng-click="showIconDialog($event.target);">选择图标</button>
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <script type="text/ng-template" id="nodes_box.html">
                                        
                                   <div class="widget-header widget-header-flat">
										<h4 class="smaller">
											<label class="radio-2">
												<input type="radio" class="ace" ng-click="checkResc(resc)" name="rescId" value="{{resc.id}}"><span class="lbl">{{resc.title}}</span>
											</label>
										</h4>
									</div>
									<div class="widget-body" ng-if="isNotEmpty(resc.subResource)">
										<div class="widget-main">
                                           <span ng-repeat="subResc in resc.subResource" ng-switch on="switchHasSub(subResc.subResource)">
											 <label class="radio-2" ng-switch-when="2">
												<input type="radio" class="ace" ng-click="checkResc(resc)" name="rescId" value="{{subResc.id}}"><span class="lbl">{{subResc.title}}</span>
											 </label>
                                             <div ng-switch-when="1" class="widget-box no-margin no-padding radio">
                                               <div ng-repeat="resc in resc.subResource" ng-include="'nodes_box.html'">
											   </div>
                                ...
                                <div class="form-group" ng-show="activeMenu.parentId > 0;">
                                    ...
                                    <div class="col-sm-9">
										<div class="tabbable">
											...
											<div class="tab-content" ng-model="activeRescs">
												<!-- 资源内容 -->
											   <div id="tab_pane_resc_{{resType.id}}" class="tab-pane {{$index == 0 ? 'active' : ''}}" ng-repeat="resType in sysResTypes">
											       	<div class="widget-box no-margin no-padding radio" ng-repeat="resc in getResc(resType)" ng-include="'nodes_box.html'">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-6 col-md-8">
                                        <textarea style="height:100px;" class="form-control basic-content-new" cols="50"
                                                  id="descn" name="descn" autocomplete="off" ng-model="activeMenu.descn"></textarea>
                                    </div>
                                </div>
