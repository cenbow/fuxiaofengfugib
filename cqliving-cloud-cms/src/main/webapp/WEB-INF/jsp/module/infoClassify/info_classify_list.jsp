				...
				<div class="row">
					...
					<form class="form-horizontal" role="form" action="info_classify_list.html" id="info_classify_FormId">
						...
						<div class="well">
                            ...
                            <cqliving-security2:hasPermission name="/module/infoClassify/change_classify_appcolumn.html">
                                <a class="btn btn-sm btn-primary" data-toggle="modal" data-target="#change_app_column" role="button">
                                  <i class="icon-share bigger-130"></i>移动新闻
                                </a>
                            </cqliving-security2:hasPermission>
						</div>
						</div>	
...
<div id="change_app_column" class="modal" tabindex="-1">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="blue bigger">移动新闻</h4>
			</div>
			<div class="modal-body overflow-visible">
				<div class="row">
					<div class="col-sm-12">
						<div class="input-group">
							<input name="columnId" type="hidden" value="${columnsId}" />
							<input name="columnName" id="columnName" type="text" placeholder="请选择所属栏目"  class="col-xs-12 form-control dropdown-toggle" value="${columnsName}" readonly/>
							<span class="input-group-btn dropdown-toggle">
								<button class="btn btn-sm btn-primary" type="button">
									<span class="caret"></span>
								</button>
							...
							<ul class="dropdown-menu dropdown-menu-right" role="menu" style="width:100%; padding:0px;">
                               <div class="dropdown-default" id="app_column_tree"></div>
                            </ul>
						</div>
                     </div>
				</div>
			...
			<div class="modal-footer">
				<button class="btn btn-sm btn-success" id="change_ok_btn">
					<i class="icon-ok"></i>确认
				</button>
				<button class="btn btn-sm btn-danger" data-dismiss="modal">
					<i class="icon-remove"></i>取消
...
<script type="text/javascript">
	...
	//缓存选中的新闻id
	var cacheInfoClassifyIds = [];
	require(["/resource/business/infoClassify/infoClassifyList.js?v=v1", "/resource/business/infoClassify/info_correlation.js?v=v1"], function(obj) {
		obj.init();
		...
		$("#change_app_column").on("click", ".dropdown-toggle,.dropdown-menu", function(e) {
			e.cancelBubble = true;
			e.stopPropagation();
			$(this).next().show();
			$("#change_app_column .dropdown-menu").show();
		});
		...
		$("body").bind("click", function(e) {
			...
			$("#change_app_column .dropdown-toggle").not("#columnName").next().hide();
		});
	...
	function pageCallBack(){
		if(!cacheInfoClassifyIds || cacheInfoClassifyIds.length<=0){
			return;
		}
		$("#table_content_page table tbody input[name=infoid]").each(function(i,n){
		     var $this = $(n);
		     var infoId = parseInt($this.val(),10);
			for(var i=0,k=cacheInfoClassifyIds.length;i<k;i++){
				var cacheId = cacheInfoClassifyIds[i];
				cacheId = parseInt(cacheId,10);
				if(cacheId == infoId){//回显
