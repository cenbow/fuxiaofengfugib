...
<!-- 		        收集信息 -->
		        ...
		        <c:if test="${fn:length(imageList) > 0 }">
			        <div class="row-fluid" style="min-height: 158px;">
						<ul class="ace-thumbnails">
							<c:forEach var="image" items="${imageList }">
								<li>
									<a href="${image.imageUrl }" data-rel="colorbox">
										<img alt="150x150" style="width:150px;height:150px;" src="${image.imageUrl }" />
									</a>
								</li>
							</c:forEach>
						</ul>
		        ...
		        <div class="widget-box <c:if test="${item.status == 2 }">hide</c:if>" id="replyDiv">
					...
					<div class="widget-body">
						<div class="widget-main">
							<form method="POST">
								<input type="hidden" name="id" value="${wzTransfer.id }">
			...
			<div class="form-group col-xs-12 text-right">
				<div class="space-4"></div>
					<c:if test="${wzTransfer.status == 1 || wzTransfer.status == 2 }">
					...
					<c:if test="${wzTransfer.status == 2 }">
						<cqliving-security2:hasPermission name="/module/wz/child/wz_question_submit.html">
							<button class="btn btn-success submitButton btn-sm" type="button" url="wz_question_submit.html?id=${wzTransfer.id }"><i class="icon-ok bigger-110"></i>提交</button>
						</cqliving-security2:hasPermission>
					</c:if>
					<c:if test="${empty wzTransfer.result}">
						<cqliving-security2:hasPermission name="/module/wz/child/wz_question_return.html">
							<button class="btn btn-warning returnButton btn-sm" type="button"><i class="icon-undo bigger-110"></i>驳回</button>
						</cqliving-security2:hasPermission>
					</c:if>
					<c:if test="${wzTransfer.status == 3 && wzTransfer.result == 3 }">
						<cqliving-security2:hasPermission name="/module/wz/child/wz_question_transfer.html">
							<button class="btn btn-success transferBtn btn-sm" type="button"><i class="icon-exchange bigger-110" data-id="${item.id }"></i>转交</button>
						</cqliving-security2:hasPermission>
					</c:if>
					<c:if test="${isLastPublish == false && wzTransfer.result == 3}">
						<cqliving-security2:hasPermission name="/module/wz/child/wz_question_publish.html">
							<button class="btn btn-success buttonPublish btn-sm" type="button" url="wz_question_publish.html?id=${wzTransfer.id }"><i class="icon-mail-forward bigger-110"></i>发布</button>
						</cqliving-security2:hasPermission>
					</c:if>
					<!-- 新增 功能-->
					<c:if test="${wzTransfer.result == 4 }">
						<cqliving-security2:hasPermission name="/module/wz/child/wz_question_publish.html">
							<button class="btn btn-primary btn-sm" type="button" onclick="editButton(this)"><i class="icon-pencil bigger-110"></i>回复编辑</button>
					...
					<c:if test="${!(!isAllowDel && wzTransfer.status == 3 && wzTransfer.result == 4)}">
						<cqliving-security2:hasPermission name="/module/wz/child/wz_question_delete.html">
							<button class="btn btn-danger btn-sm" id="deleteButton" type="button" url="wz_question_delete.html?id=${wzTransfer.id }"><i class="icon-trash bigger-110"></i>删除</button>
						</cqliving-security2:hasPermission>
					</c:if>
					<button class="btn btn-sm btn-primary" type="button" onclick="javascript:void(0);" backRestoreParam="backRestoreParam" back_url="/module/wz/child/wz_question_list.html">
	...
	<div class="modal-dialog">
	    <div class="modal-content">
	        <form class="form-horizontal form" method="post" id="transferForm">
	        	<input type="hidden" value="${wzTransfer.id }" name="id">
	...
	<div class="modal-dialog">
	    <div class="modal-content">
	        <form class="form-horizontal form" method="post">
	            ...
	            <div class="modal-header">
	                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
	                <h3 class="ng-binding">确认驳回</h3>
	            </div>
	            <div class="modal-body">
	                <div class="form-group">
	                	<textarea class="form-control col-sm-10" name="content" rows="3" required placeholder="请输入驳回原因"></textarea>
	                </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary submitBtn"><i class="icon-ok bigger-110"></i>确定</button>
...
require(['/resource/business/wz/child/wzQuestionView.js'], function(obj){
	obj.init();
});
function editButton(me){
	...
	//$('#replyDiv').find('textarea').removeAttr('disabled');
	uEditor.setEnabled();
}
</script>