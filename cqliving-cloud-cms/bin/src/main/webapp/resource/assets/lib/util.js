define(['bootstrap'], function($){
    ...
    require(['bootstrap.switch'], function($){
        $(function(){
            ...
            var json=[];
            $.ajax({
                url: "/cms/site/ajax_nav_url_list.html",
                async:false,
                cache:false,
                dataType:"json",
            ...
            //console.debug("json is array:%s, url list:%o", $.isArray(json), json);
            $('input[type="checkbox"][data-nav-url]').each(function(){
                var elm=$(this);
                //url 是否在导航中
                var b = false;//$.inArray($(this).data("nav-url"), json) >= 0;
                var thisUrl=elm.data("nav-url");
                ...
                //console.debug("state:%s, this nav url:%s", b, thisUrl);
                elm.show().on('switchChange.bootstrapSwitch', function(event, state) {
                    var postJson={
                        navUrl: $(this).data("nav-url"),
                        navTitle:$(this).data("nav-title"),
                        status:state ? 1 : 0
                    };
                    //console.debug("url:/cms/site/ajax_save.html , postData:%o", postJson);
                    $.post("/cms/site/ajax_save.html", postJson).success(function(result){
                        if(result.success){
                            cms.message((state ? "加入导航":"取消导航")+"成功", "", "success");
                        }else{
                            cms.message((state ? "加入导航":"取消导航")+"失败", "", "error");
                        }
                    });

                }).bootstrapSwitch({state:b});
            });
        });
    });*/
    //end 加入导航开关
	...
	module.emotion = function(elm, target, callback) {
		require(['jquery.caret', 'bootstrap', 'css!/resource/css/emotions.css'],function($){
			$(function() {
	...
	module.iconBrowser = function(callback){
		...
		var modalobj = module.dialog('请选择图标',['/resource/themes/utility/icon.htm'],footer,{containerName:'icon-container'});
		modalobj.modal({'keyboard': false});
	...
	module.emojiBrowser = function(callback){
		var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>';
		var modalobj = module.dialog('请选择表情',['/resource/themes/utility/emoji.htm'],footer,{containerName:'icon-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-dialog').css({'width':'70%'});
		modalobj.find('.modal-body').css({'height':'70%','overflow-y':'scroll'});
		modalobj.modal('show');
	...
	module.image = function(val, callback, opts) {
		var content = 	'<ul class = "nav nav-tabs" style="margin:auto -10px;padding:0 10px;">'+
						...
						'<div class = "tab-content form-horizontal" style="padding:20px 0;">'+
							...
							'<div class="tab-pane" id="image_upload">'+
                                ...
                                '<form action="/global/upload/save.html" enctype="multipart/form-data" method="post" target="__image_file_uploader">'+
									'<div class="form-group">' +
		...
		modalobj.find('button.btn-browser').on('click', function(){
			...
			window.imageBrowser = {
				...
				browser: function(path){
                    ...
                    dialog.find('.modal-body').load('/global/upload/list.html?options='+opts+'&type=image&callback=imageBrowser');
				},
				select: function(r) {
					callback({filename: r.filename, url: r.url});
					modalobj.modal('hide');
				},
				'delete': function(obj, file){
					$.get('/global/upload/delete.html?url=' + file).success(function(dat){
                        console.log(dat);
						if(dat == 'success') {
							console.log(file);
                            $(obj).parent().remove();
		...
		modalobj.find('button.btn-primary').on('click', function(){
			if(modalobj.find('.nav.nav-tabs li').eq(0).hasClass('active')) {
                ...
                if(modalobj.find(':file[name="file"]').val()==""){
                    module.message("请选择图片附件",'','error');
                    return ;
                } else {
	...
	*/
	module.ajaxshow = function(url, title, options, events) {
		...
		var footer = (typeof events['confirm'] == 'function' ? '<a href="javascript:void(0);" class="btn btn-primary confirm">确定</a>' : '') + '<a href="javascript:void(0);" class="btn  btn-default" data-dismiss="modal" aria-hidden="true">关闭</a><iframe id="_formtarget" style="display:none;" name="_formtarget"></iframe>';
	...
	module.createHideForm = function(url, formName, params) {
		var form = $('<form action="' + url + '" method="POST"></form>');
		if(formName && formName != null && typeof formName === 'string') {
			$.each($('#'+ formName).serializeArray(), function(i, item){
    ...
    module.isAuthCode = function(code){
        return "CQLIVING_SECURITY2_NOT_AUTH"== code;
    };
