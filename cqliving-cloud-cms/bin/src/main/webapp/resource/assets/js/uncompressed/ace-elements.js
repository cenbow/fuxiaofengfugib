...
(function($ , undefined) {
	$.fn.ace_colorpicker = function(options) {
		...
		this.each(function() {
			...
			var colors = '';
			var color = '';
			$(this).find('option[data-skin='+($.cookie('ace-skin') ? $.cookie('ace-skin') : 'default')+']').prop('selected', true);
			...
			$(this).hide().find('option').each(function() {
				...
				colors += '<li><a class="'+$class+'" href="#" style="background-color:'+this.value+';" data-color="'+this.value+'"></a></li>';
					...
					$.cookie('ace-skin', null);
					$.cookie('ace-skin', $(this).find('option:selected').data('skin'), {expires: 365, path: '/'});
