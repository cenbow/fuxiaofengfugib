...
public class PortalSimplePaging extends TagSupport {
	 ...
	 * 表单提交
	 */
	private static final String PAGE_SCRIPT = "<script language=\"JavaScript\">"
	...
	private String makeButton(String label, String value, boolean disable) {
		String style = "type:button;" + "value=\"\"" + " class=\"" + label
				+ "\"" + " onclick:goToByLabel(document." + this.name + ",'"
				+ value + "');" + (disable ? "disabled:disabled" : "");
		String button = this.makeStartTag("input", this.buttonParam + ";"
				+ style);
		return button;
	}

	private String makeHref(String pageNo) {
		String href = "<a href=\"javascript:goToByLabel(document.getElementById('"
				+ this.name + "'),'" + pageNo + "')\"";
	...
	private String makeSelector(String style, Map<Object, Object> options, Object selected) {
		...
		for (Iterator<Object> ite = options.keySet().iterator(); ite.hasNext();) {
			...
			buf.append("<option value=\"" + v + "\""
					+ (v.equals(selected) ? "selected" : "") + ">" + k
					+ "</option>");
		}
	...
	public int doStartTag() throws JspException {
		...
		try {
			if (this.checkMethod == null || this.checkMethod.equals("")) {
				out.println(this.PAGE_SCRIPT.replaceFirst("&checkMethod&",
						"true"));
			} else {
				out.println(this.PAGE_SCRIPT.replaceFirst("&checkMethod&",
						this.checkMethod));
			...
			/** 当前最大可能页数 */
			if (totalPage > 1) {
				int maxCurrentPage = currentPage
						+ Integer.parseInt(this.showPageNo);
			...
			out.println(this.makeHidden("previous", "previous",
			...
			out.println(this.makeHidden("currentPage", "currentPage",
					String.valueOf(pageInfo.getCurrentPage())));
			out.println(this.makeHidden("totalPage", "totalPage",
					String.valueOf(pageInfo.getTotalPage())));
