...
public abstract class AbstractEntityController<T extends AbstractEntity, M extends EntityService<T>>
	...
	private static final Logger logger = LoggerFactory
			.getLogger(AbstractEntityController.class);
	...
	public String importXls(Model model, HttpServletRequest request,
			HttpServletResponse response) {
		try {
			Map<String, UploadResult> uploadResults = doUpload(request);
			List<String[]> lines = doImportLoadXls(uploadResults,
	 ...
	 * 功能: <li>自动完成文件上传，读取，反序列化框架和保存。 <li>支持文件读取后，转换前的格式检查和相关子类处理:onImpCsvLoad
	 * <li>只需要实现每行数据的反序列化，自动反序列化数据到实体集合 <li>
	 * 支持转换为实体集合，批量保存前进行合法性检查和相关类处理：onImpCsvUnMarshal
	 * 
	 * @param request
	 * @param response
	 * @param model
	...
	public String importCsv(Model model, HttpServletRequest request,
			HttpServletResponse response) {
		try {
			Map<String, UploadResult> uploadResults = doUpload(request);
			List<String[]> lines = doImportLoadCsv(uploadResults,
					getCharsetName(request));
			beforeUnmarshal(lines);
			List<T> entities = unmarshal(lines);
			afterUnmarshal(entities);
	...
	public String exportXls(Model model, HttpServletRequest request,
		...
		try {
			...
			boolean needResponseToExportViewPage = onExportXls(list, request,
					response);
	...
	protected void doExportExcelHeader(HttpServletRequest request,
		...
		try {
			fileName = new String(fileName.getBytes(getCharsetName(request)),
					"ISO-8859-1");
		} catch (Exception e) {
			// ig
		}
	 ...
	 * @return
	 */
	protected boolean onExportXls(List<T> list, HttpServletRequest request,
			HttpServletResponse response) {
	...
	protected Map<String, UploadResult> doUpload(HttpServletRequest request)
		...
		for (Map.Entry<String, MultipartFile> entry : multipartFiles.entrySet()) {
			...
			if (mfile.getSize() > uploadConfig.getMaxSize()) {
				throw new RuntimeException("文件[" + mfileName + "]大小操作限制，最大限制:"
			...
			if (!StringUtils.containsIgnoreCase(
				...
				throw new RuntimeException("文件[" + mfileName + "]类型不支持，支持类型:"
						+ uploadConfig.getAllowExtentions());
			}

			if (uploadConfig.isUseMemery()) {
				// 内存方式，不转存到服务器存储，直接返回流给调用端
				result = new UploadResult(entry.getKey(), mfileName,
						mfile.getSize(), mfile.getInputStream());
			} else {
				// 转存到服务器，返回服务器文件
				File destFile = new File(getUploadFileName(mfileName,
						uploadConfig));
