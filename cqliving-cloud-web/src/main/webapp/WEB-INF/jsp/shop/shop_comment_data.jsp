...
<c:forEach items="${dataList}" var="obj">
	<li replyid="${obj.id}">
		...
		<c:if test="${obj.passiveRelpsyStatus eq 3 and not empty obj.passiveReplyName}">
	     	<div class="comment_list_reply">回复<span class="comment_list_reply_user">${obj.passiveReplyName}</span>的评论：${obj.sourceTitle}</div>
	    </c:if>
	</li>
</c:forEach>