...
//	/** 当前页是否正在进行*/
//	exports.submitting = false;
	//后缀名是否符合规范
	exports.checkSuffix = function(config,inputId){
		//获取上传文本框的值
	 ...
	 * 重写jquery   load方法
	 */
	exports.load=function(obj,url,dada,fn){
	 ...
	 */
	exports.ajaxOperate = function(url,el,data, callback,args){
		if(el){
			el = $(el);
			if(el){
...
//	        }
	 ...
	 */
	exports.checkFileSize = function(maxsize,fileElementId){
...
//		var tipMsg = "您的浏览器暂不支持计算上传文件的大小，确保上传文件不要超过2M，建议使用IE、FireFox、Chrome浏览器。";
	 ...
	 * @param {function} callbaxk 回调函数 
	 */
	exports.ajaxFileUpload = function(url,fileElementId,callback){
		url = addIsAjax(url);
		if(false){
			//弹出遮罩层
		}
		...
		$.ajaxFileUpload({
			...
			success : function beforeCallback(data, status){
				if(data==QZ_LOGIN_REPEAT){//重复登录
					window.location.href = "/index.html";
				} else if(data==QZ_NOT_LOGIN){//未登录
	                window.location.href = "/login.html";
	            }else if(callback!=null){
				    callback(data, status);
				    if(false){
	 ...
	 */
	exports.standardPost = function(url,args){
