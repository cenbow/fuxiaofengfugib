package com.cqliving.tool.common.util.code;

import java.util.HashMap;
import java.util.Map;

public class Number36Utils {

	private static final String[] SEED_NUM_ARRAY = { "0", "1", "2", "3", "4",
	...
	public static String dec2any(String dec, int targetDevider) {
		if (targetDevider == 10)
			return dec;
		if (targetDevider > RADIX_36 || targetDevider < 2 || null == dec
				|| "".equals(dec.trim()))
			return null;
		try {
	 ...
	 * @return 目标进制的字符串
	 */
	public static String any2any(String sourceNum, int soruceDevider,
			int targetDevider) {
		if (targetDevider > RADIX_36 || targetDevider < 2
				|| soruceDevider > RADIX_36 || soruceDevider < 2)
			return null;
	...
	public static String any2dec(String sourceNum, int soruceDevider) {
		if (soruceDevider == 10)
			return sourceNum;
		if (soruceDevider > RADIX_36 || soruceDevider < 2 || null == sourceNum
		...
		try {
			...
			for (int i = 0; i < sourceNum.length(); i++) {
				...
				if (k >= soruceDevider)
					return null;
				sum += k
						* Math.pow(soruceDevider, sourceNum.length() - (i + 1));
			}
...
//    	2126
//    	999999
/*    	for (int i = 0; i <= RADIX_36; i++) {
    		System.out.println(i+ "==");
    		System.out.println(any2any(i+"", 10, RADIX_36));
    	}*/
//    	Map m = new HashMap();
//    	System.out.println(m.size());
//    	long t1 =System.currentTimeMillis();
...
//    	System.out.println(m.size());
    	...
    	//for  (int i=0;i<38;i++){
    	System.out.println(Number36Utils.any2any(System.currentTimeMillis()+"", Number36Utils.RADIX_10, Number36Utils.RADIX_36));
