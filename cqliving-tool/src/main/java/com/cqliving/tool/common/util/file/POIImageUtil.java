...
/**
 * Title:
 * <p>Description:ppt pptx pdf转图片工具类</p>
 * Copyright (c) CQLIVING 2016
 * @author fuxiaofeng on 2016年12月12日
 */
public class POIImageUtil {
	
    private static final Logger logger = LogManager.getLogger(POIImageUtil.class);
    public static final int DEFAULT_WIDTH = 800;
    public static final int DEFAULT_HEIGHT = 600;

    /**
     * Title:
     * <p>Description:转图片方法，返回生成的图片列表集合</p>
     * @author fuxiaofeng on 2016年12月12日
     * @param pptPath
     * @param descImgPath
     * @return
     * @throws Exception
     */
    public static List<String> convert(String pptPath,String descImgPath) throws Exception {
        
    	//创建路径
        FileUtils.validDirectory(new File(descImgPath).getParent(), true);
        String ext = getExtension(pptPath);
        if (".pptx".equalsIgnoreCase(ext)) {
            return convertPPTX(pptPath,descImgPath);
        } 
        if (".ppt".equalsIgnoreCase(ext)) {
           return convertPPT(pptPath,descImgPath);
        } 
        if(".pdf".equalsIgnoreCase(ext)){
          return convertPdf(pptPath,descImgPath);
        }
        throw new BusinessException("-----------------不支持的文件格式-------------");
    }

    private static List<String> convertPPT(String pptPath,String descImgPath) throws IOException {
        ...
        for (int index = 0; index < slide.length; index++) {
        	String path = appendIndex(descImgPath,index);
            drawPPTSlide(slide[index],pgsize,path);
            list.add(path);
        }
        return list;
    }

    private static void drawPPTSlide(Slide slide,Dimension pgsize,String despath) throws IOException {
        setFont(slide);
        BufferedImage buffimg = createBufferImg(pgsize);
        slide.draw((Graphics2D)buffimg.getGraphics());
        saveImage(buffimg,despath);
    }

    private static List<String> convertPPTX(String pptPath,String descImgPath) throws IOException {
        FileInputStream is = new FileInputStream(pptPath);
        ...
        for (int index = 0; index < slide.length; index++) {
        	String path = appendIndex(descImgPath,index);
            drawPPTX(slide[index],pgsize,path);
            listPath.add(path);
        }
        return listPath;
    }

    private static void drawPPTX(XSLFSlide slide,Dimension pgsize,String descImgPath) throws IOException {
        setFont(slide);
        BufferedImage buffimg = createBufferImg(pgsize);
        slide.draw((Graphics2D)buffimg.getGraphics());
        saveImage(buffimg,descImgPath);
    }

    public static List<String> convertPdf(String pptPath,String descImgPath) throws Exception {
        PDDocument document = PDDocument.load(new File(pptPath));
        PDFRenderer pdfRender = new PDFRenderer(document);
        List<String> list = Lists.newArrayList();
        int pageNum = document.getNumberOfPages();
        for (int index = 0; index < pageNum; index++) {
            String path = appendIndex(descImgPath,index);
            BufferedImage image = pdfRender.renderImageWithDPI(index,96); 
    ...
    private static void saveImage(BufferedImage buffimg,String descImgPath) throws IOException {
        OutputStream out = new FileOutputStream(descImgPath);
    ...
    private static int doubleToInt(Double d){
    	...
    	return StringUtil.stringToInteger(String.valueOf(d));
    }
    
    private static BufferedImage createBufferImg(Dimension pgsize) {
    	
    	BufferedImage img=null;
    	Image smallImg=null;
    	if(null == pgsize || pgsize.getHeight()<=0 || pgsize.getWidth() <=0){
    		 img = new BufferedImage(DEFAULT_WIDTH, DEFAULT_HEIGHT, BufferedImage.TYPE_INT_RGB);
             smallImg = img.getScaledInstance(DEFAULT_WIDTH, DEFAULT_HEIGHT, Image.SCALE_SMOOTH);
    	}else{
    		 img = new BufferedImage(doubleToInt(pgsize.getWidth()), doubleToInt(pgsize.getHeight()), BufferedImage.TYPE_INT_RGB);
             smallImg = img.getScaledInstance(doubleToInt(pgsize.getWidth()), doubleToInt(pgsize.getHeight()), Image.SCALE_SMOOTH);
    	}
    ...
    private static String appendIndex(String path,int index){
    	...
    	str.append("_").append(index).append(getExtension(path));
    	return str.toString();
    }
    ...
    public static void main(String[] args) {
    	...
    	try {
    		List<String> list = POIImageUtil.convert("d:/Apesk_report(2).pdf","d:/poiimageUtil/Apesk_report.jpg");
