 ...
 * @author huxiaoping on 2016-12-27 
 * 参考 http://www.open-open.com/code/view/1420373343171/
 * 
 */  
...
public class IdcardUtil {
     ...
     * @parameter 
     * @return
     * @author huxiaoping 2016年12月27日下午4:24:19
     */
    public static Boolean IDCardValidate(String IDStr) {
        ...
        if (IDStr.length() != 15 && IDStr.length() != 18) {
            throw new BusinessException(-1,"身份证号码长度应该为15位或18位。");
        }
        ...
        if (isNumeric(Ai) == false) {
            throw new BusinessException(-1,"身份证15位号码都应为数字 ; 18位号码除最后一位外，都应为数字。");
        }
        ...
        if (isDate(strYear + "-" + strMonth + "-" + strDay) == false) {  
            throw new BusinessException(-1,"身份证出生日期无效。");
        }
        ...
        try {  
            if ((gc.get(Calendar.YEAR) - Integer.parseInt(strYear)) > 150  
                ...
                throw new BusinessException(-1,"身份证生日不在有效范围。");
            }  
        ...
        if (Integer.parseInt(strMonth) > 12 || Integer.parseInt(strMonth) == 0) {  
            throw new BusinessException(-1,"身份证月份无效。");
        }  
        if (Integer.parseInt(strDay) > 31 || Integer.parseInt(strDay) == 0) {  
            throw new BusinessException(-1,"身份证日期无效。");
        }
        ...
        if (areacode.get(Ai.substring(0, 2)) == null) {  
            throw new BusinessException(-1,"身份证地区编码错误。");
        }  
          
        if(isVarifyCode(Ai,IDStr)==false){  
            throw new BusinessException(-1,"身份证校验码无效，不是合法的身份证号码。");
        }
